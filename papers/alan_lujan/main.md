...
`[[Is there a way to give a nod to PyTorch and TensorFlow for having call-backs, hooks, and function wrappers to allow a user to swap out an optimization function/module mid-stream? It does not cover all the use cases of the `multinterp` package, but some effort went into developing a layered API to cover varying use cases.  NumPy also has strductured datatype, that can be used for irregular data structures and hierarchical data structures.  It can be seen as an attempt to provide flexible user interface, even though the aim and scope if not the same as 'multinterp' package. General readers my appreciate having some context, and how this package can be viewed as a further development of previous efforts at flexible user interface.]]`
